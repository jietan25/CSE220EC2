 ...
 * PCODE:
 * Assert pList is not null.
 * Define index as int <- DListNodeGetData(DListFindData(pList, pBefore))
...
{
    assert(pList);
     DListNode *index_node, *new_node, *prev_index;/* = DListNodeAlloc(pData, DListGetTail(pList), NULL);*/
    if (DListIsEmpty(pList) || pIndex < 0 || pIndex >= DListGetSize(pList)) {
        return NULL;
    }
    if ((index_node = DListFindIndex(pList, pIndex))== NULL){
        return NULL;
    }
    ...
    )
{
    ...
    if (pNode == DListGetHead(pList)){
        ...
        if (DListGetHead(pList)!=NULL){
            ...
            DListSetTail(pList, DListNodeGetPrev(pNode));
            if (DListGetTail(pList)!= NULL){
                    DListNodeSetNext(DListGetTail(pList), NULL);
            }
    }

    else {
        if (DListNodeGetPrev(pNode) != NULL) {
            DListNodeSetNext(DListNodeGetPrev(pNode), DListNodeGetNext(pNode));
            if (DListNodeGetNext(pNode)!= NULL){
                DListNodeSetPrev(DListNodeGetNext(pNode), DListNodeGetPrev(pNode));
            }
        }
    }
