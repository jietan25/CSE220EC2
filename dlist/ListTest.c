    ...
    )
{
    ...
    fprintf(gFout, "copies %s to %s\n", srclistname, dstlistname);

   /* else fprintf(gFout, "failed to copy %s to %s\n", srclistname, dstlistname);*/
}
    ...
    )
{
    ...
    found_node = DListFindIndex(list, index);
    if (found_node) fprintf(gFout, "found %d in %s\n", index, listname);
    else fprintf(gFout, "failed to find at %d in %s\n", index, listname);
}
    ...
    )
{
    ...
    if (DListInsertBefore(list, before, data)) {
        ...
        fprintf(gFout, "failed to insert %d before %d\n", data, before);
    }
}
    ...
    )
{
    ...
    if (DListInsertIndex(list, index, data)) {
        fprintf(gFout, "inserted new node with data %d at %d of %s\n", data, index, listname);
    } else {
        fprintf(gFout, "failed inserted new node with data %d at %d of %s\n", data, index, listname);
    }
}
    ...
    )
{
    ...
    fscanf(gFin, "%s%d", listname, &index);
    list = ListManGetList(listname);
    if (DListRemoveIndex(list, index)) fprintf(gFout, "removed node at index %d from %s\n", index, listname);
    else fprintf(gFout, "failed to remove node at index %d from %s\n", index, listname);
}
